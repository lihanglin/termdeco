...
;; $Id: xgtags.el,v 1.37 2006/03/30 20:52:14 mg Exp $
...
(defun xgtags-make-tag (query file line match)
  (vector query file line match))

(defun xgtags-tag-query (tag)
  (aref tag 0))
...
(defun xgtags-find-tag-from-here ()
  ...
  (interactive)
  (let ((tagname (xgtags-token-at-point))
	(flag (cond ((not (xgtags-function-p)) "s")
		    ((xgtags-definition-p) "r")
		    (t ""))))
    (when tagname
...
(defun xgtags-find-tag-by-event (event)
  ...
  (let (tagname flag)
	...
	(select-window (posn-window (event-end event)))
      ...
      (setq flag (cond ((not (xgtags-function-p)) "s")
		       ((xgtags-definition-p) "r")
		       (t  ""))))
    (when tagname
      (xgtags-goto-tag tagname flag))))
...
the current buffer."
  ...
  (let ((current-file nil))
    (dolist (tag tags)
      (let ((query (xgtags-tag-query tag))
	    (file (xgtags-tag-file tag))
	    (line (xgtags-tag-line tag))
	...
	(unless (equal current-file file)
	  ...
	  (insert "\n"))
	(let ((start (point)))
	  (xgtags-insert-with-face query 'xgtags-match-face)
	  (insert "[")
